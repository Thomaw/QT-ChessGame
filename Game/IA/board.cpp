...
// Création d'un nouveau Namespace pour pouvoir différencier les fonctions de l'IA et
// être sûr de ne pas avoir de conflit entre les fonctions
namespace Chess {
 ...
 * @return
 *
 * Positionnement des pièces à partir de la FEN
 */
Board Board::fromFEN(std::string fenRecord){
    ...
    for ( i = 0; i < fenRecord.size() && fenRecord[i] != ' '; ++i) {
        ...
        if (ch == '/') {
            ...
            // Positionne les pièces sur le plateau de jeu virtuel
            char name = std::toupper(ch);
            if (name == 'P') {piece = Piece(Piece::Pawn, std::isupper(ch) ? Piece::White : Piece::Black);}
            else if (name == 'N') { piece = Piece(Piece::Knight, std::isupper(ch) ? Piece::White : Piece::Black);}
            else if (name == 'B') { piece = Piece(Piece::Bishop, std::isupper(ch) ? Piece::White : Piece::Black);}
            else if (name == 'R') { piece = Piece(Piece::Rook, std::isupper(ch) ? Piece::White : Piece::Black);}
            else if (name == 'Q') { piece = Piece(Piece::Queen, std::isupper(ch) ? Piece::White : Piece::Black);}
            else if (name == 'K') { piece = Piece(Piece::King, std::isupper(ch) ? Piece::White : Piece::Black);
            }
...
/**
 * @brief Board::isSquareAttacked
 * @param square
 * @param attackingSide
 * @return
 *
 * Fonction vérifiant si un carré précis est attaqué
 */
bool Board::isSquareAttacked(Coord square, Piece::Color attackingSide){
    ...
    // Mouvements du cavalier dans le sens des aiguilles d'une montre
...
/**
 * @brief Board::isKingAttacked
 * @param side
 * @return
 *
 * Fonction vérifiant si le roi est attaqué
 */
...
/**
 * @brief Board::possibleMoves
 * @param from
 * @return
 *
 * Calcul de tous les mouvements possibles de l'algorithme
 */
Vector<Move> Board::possibleMoves(const Coord from){
    ...
    if (piece.isKnight() ) {
        ...
        // Mouvements du chevalier dans le sens des aiguilles d'une montre
        static const sint8 knightOffsets[8][2] = { {+1,+2}, {+2,+1}, {+2,-1}, {+1,-2}, {-1,-2}, {-2,-1}, {-2,+1}, {-1,+2} };
    ...
    // Supprime tous les mouvements qui mettent le roi en échec
    for (size_t i=0; i < movesList.size(); ++i) {
        ...
        if (isKingAttacked(piece.color() )) {
            movesList[i--] = movesList.back();  // supprimer i des mouvements possibles
            movesList.pop_back();
        }
 ...
 * @return
 *
 * Mouvements possibles selon la couleur du joueur
 */
 ...
 * @param move
 *
 * Fonction pour réaliser le mouvement sur l'échiquier
 */
 ...
 * @brief Board::unmake
 *
 * Fonction pour annuler le mouvement de l'échiquier
 */
void Board::unmake(){
    if (movesDone.size() == 0)
        return;
