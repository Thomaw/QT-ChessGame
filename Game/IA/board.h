...
public:
    ...
    inline bool isOccupied(Coord coord) const {return !squares[coord].isEmpty();}
    ...
    inline Piece piece(Coord coord) const {return squares[coord];}
    inline Piece::Color side() const {return m_sideToMove;}
    inline Piece & operator[](Coord square) {return squares[square];}
};
